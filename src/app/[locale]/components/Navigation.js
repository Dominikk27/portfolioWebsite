...
const Navigation = () => {
  ...
  return (
    <div className={`fixed w-full px-[2rem] py-2 flex flex-row-reverse md:flex-row justify-between items-center transition-all duration-500 ease-in z-[1000]
      ...
      {/* MOBILE HAMBURGER */}
      <div className="md:hidden w-[2.9rem] h-[2.9rem] rounded-[0.9rem] bg-accent flex justify-center items-center z-[105]" onClick={() => setIsOpen(!isOpen)}> 
        { isOpen ? (
      ...
      <AnimatePresence>
        { isOpen && (
          <motion.div
            ...
            className="fixed top-0 right-0 w-2/3 h-screen bg-black/10 backdrop-blur-md flex flex-col items-center justify-center gap-6 text-white md:hidden z-[100]"
          >
            {navLinks.map((link) => (
              <a key={link.labelKey}
                href={link.href}
